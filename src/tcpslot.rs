...
//! slot target. A persistent STUN-over-TCP holder from the same tuple
...
//! (results/RESULTS-2026-09-13-c3.md), so the holder refreshes at a
//! cadence strictly under the lower bound. The holder's XOR-MAPPED is
//! the slot's external TCP tuple, published per slot on churn. The
//! holder lifecycle is the Kani-proven state machine. The fw4 TCP input
...
// ---- holder lifecycle (the Kani-provable part) ----

/// The mapping holder state machine. Held means a live STUN-over-TCP
...
impl HolderState {
    pub fn is_held(&self) -> bool {
        *self == HolderState::Held
    }

    /// A connection error kills the held mapping: Dead.
    pub fn on_error(&mut self) {
        if *self == HolderState::Held {
            *self = HolderState::Dead;
        }
    }

    /// Re-establishment restores the mapping: Held.
...
mod proofs {
    ...
    fn cadence_under_bound() {
        ...
        // dies by 300 s on the measured node/session. The holder refresh
    ...
    fn holder_cycles_through_both_states_without_unknown() {
        ...
        for i in 0..3 {
            ...
            assert!(match h {
                HolderState::Held => i % 2 == 0,
                HolderState::Dead => i % 2 == 1,
            });
        }
    }
}